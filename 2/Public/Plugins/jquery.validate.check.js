...
jQuery.validator.addMethod("bankcode",function(value,element){
    return this.optional(element)||/^\d{8,19}$/.test(value);
},'请输入正确的银行卡号');
...
jQuery.validator.addMethod("isIDCard",function(value,element){
    return this.optional(element)||/^(\d{18,18}|\d{15,15}|\d{17,17}[xX]{1})$/.test(value);
},"请正确填写身份证号！");
...
jQuery.validator.addMethod("regEmail",function(value,element,params){	
	if($.trim(value)==''){		
		$.validator.messages.regEmail = "请输入您常用的邮箱";
		return false;
	}
	if(!/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(value)){
		$.validator.messages.regEmail = "请输入正确邮箱地址";
		return false;
	}
	 var url=$(element).attr("regEmail");
	 var changeUrl=	url+value;     
	  var html = $.ajax({
 					url:changeUrl,
  					async: false
 				 }).responseText;
	if(html=="1"){return true;}
	 ...
	 $.validator.messages.regEmail = "邮箱已注册,请用其它邮箱注册";		 
	 return false; 
	}
},$.validator.messages.regEmail);
...
jQuery.validator.addMethod("notchar",function(value,element){   
	return this.optional(element)||( /^[\u4E00-\u9FA5A-Za-z0-9_\,\.\-\s]+$/.test(value));
},"请不要输入非法字符");
...
jQuery.validator.addMethod("regex",function(value,element,params){	
	...
	msg=msg?msg:"输入不合法";		
	$.validator.messages.regex=msg;
	return false;	 
},$.validator.messages.regex);

jQuery.validator.addMethod("exist",function(value,element,params){	  
	 var exist_url=$(element).attr("exist");
	 var msg=$(element).attr("msg");	 
	 var changeUrl=	exist_url+value;	 
	...
	if(!json){
		$.validator.messages.exist=msg?msg:"服务器未响应";
		return false;
	}
	if(json.substr(0,1)!="{"){		
		$.validator.messages.exist = json?json:(msg?msg:"服务器未响应");
		return false; 	
	}
	var result=jQuery.parseJSON(json);			 
	if(result.status=="1")	return true;
	else{
	 $.validator.messages.exist = result.info;		 
	 return false; 
	}
},$.validator.messages.exist);
...
jQuery(document).ready(function($){
	...
	$.each(validateObj,function(){
		if($(this).attr("validate")=="true"){
			...
			var validateinfo=$(this).attr("validate");
		}
		if(validateinfo){
		...
		$(this).validate({
			...
			errorPlacement: function(error, element) {
				...
				if(element.attr("msg"))	error.html(element.attr("msg"));		
