	...
	if($.Datatype){
		...
		$.extend($.Datatype,{
			...
			*/
			...
			//奖金限制
			"award_check": function(gets,obj,curform,datatype){
				var regnum = /^\d+$/;
				if(gets != ''){
					if(!regnum.test(gets)){return "请输入数字！"};
					var gets = parseFloat(gets);
					var input = parseFloat($(obj).attr("lt"));
					if(gets > input || gets < 0){
						return "使用奖金金额不得大于"+$(obj).attr("lt");
					}
