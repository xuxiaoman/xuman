...
// +----------------------------------------------------------------------
// $Id: Http.class.php 2207 2011-11-30 13:17:26Z liu21st $
 ...
 * @subpackage  Net
 * @author    liu21st <liu21st@gmail.com>
 * @version   $Id: Http.class.php 2207 2011-11-30 13:17:26Z liu21st $
 +------------------------------------------------------------------------------
 */
class Http
{//类定义开始
    static private $cookies = array();
    static private $last_header;
    static private $http_status;
    static $configs = array("timeout" => 0);
    ...
    static public function curl_download($remote,$local) {
     ...
     /**
     * 使用curl 通过 HTTP 协议直接访问(采集)远程文件
     * @param string $url 远程URL
     * @param array $post post的内容,数组
     * @param array $cookie 携带cookie访问,该参数是cookie内容
     * @return string
     */
    static public function getCurl($url = '', $post = array(), $cookie = array()) {
        $conn = curl_init($url);
        curl_setopt($conn, CURLOPT_TIMEOUT, self::$configs['timeout']);
        ...
        self::$cookies = array_merge(self::$cookies, $cookie);
        if (self::$cookies) {
            curl_setopt($conn, CURLOPT_COOKIE, join(';', self::$cookies));
        }
        if ($post) {
        ...
        self::$http_status = $http_status[1];
        return $body;
    }
