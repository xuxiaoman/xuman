<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN"
    ...
    </script></head><body width="100%"><div id="result" class="result none"></div><div class="mainbox"><div id="nav" class="mainnav_title"><ul><a class="on" href="<?php echo U('index');?>">路线类型</a> |
        <a href="<?php echo U('add');?>">添加</a></ul></div><form name="myform" id="myform" action="" method="post"><div class="table-list"><table cellspacing="0" cellpadding="0" width="100%"><thead><tr><th><a href="javascript:void(0);" onclick="sel_all();">全选</a></th><th style="text-align: left;">名称</th><th>英文名称</th><th>排序</th><th>操作</th></tr></thead><tbody><?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="ulbc" id="tr_<?php echo ($vo["id"]); ?>"><td id="td_sel"><input type="checkbox" class="sel" id="sel_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["id"]); ?>"></td><td style="text-align: left;"><input type="text" name="names_<?php echo ($vo["id"]); ?>" id="names_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["names"]); ?>"></td><td><input type="text" name="page_names_<?php echo ($vo["id"]); ?>" id="page_names_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["page_names"]); ?>"></td><td><input type="text" name="sort[<?php echo ($vo["id"]); ?>]" id="sort_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["sort"]); ?>"></td><td><a href="javascript:void(0);" onclick="ajax_save('<?php echo ($vo["id"]); ?>')">保存</a> |
                        <a href="#" onclick="javascript:if(confirm( '确定要删除该条记录吗？ '))ajax_del(<?php echo ($vo["id"]); ?>);">删除</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></div><div class="btn" style="display: none;"><input type="button" class="button" name="dosubmit" value="删除" onclick="myform.action='<?php echo U('deleteall');?>';return confirm_deleteall()"><input type="button" class="button" name="dosubmit" value="排序" onclick="myform.action='<?php echo U('sort_list');?>';$('#myform').submit();"></div></form><div class="page"><?php echo ($page); ?></div><script language="javascript">    function ajax_save(id){
