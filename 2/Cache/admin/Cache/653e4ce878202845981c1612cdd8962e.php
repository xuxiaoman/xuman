<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN"
...
</script><div id="nav" class="mainnav_title"><ul><a class="on" href="<?php echo U('show_list');?>">点评管理</a></ul></div><form name="myform" id="myform" action="" method="post"><div class="table-list"><table cellspacing="0" cellpadding="0" width="100%"><thead><tr><th><a href="javascript:void(0);" onclick="sel_all();">全选</a></th><th>印象编号</th><th>用户名</th><th>满意度</th><th>评价时间</th><th>操作</th></tr></thead><tbody><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="ulbc"><td id="td_sel" align="center"><input type="checkbox" class="sel c_selected" id="sel_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["id"]); ?>" name="items[<?php echo ($vo["id"]); ?>]"></td><td align="center"><?php echo ($vo["id"]); ?></td><td align="center"><?php echo ($vo["order_id"]); ?></td><td align="center"><?php echo ($vo["point"]); ?></td><td align="center"><?php echo (f_date($vo["create_time"])); ?></td><td align="center" id="control_<?php echo ($vo["id"]); ?>"><?php if(($vo["status"]) == "5"): ?><a href="javascript:void(0);" onclick="fa_award(<?php echo ($vo["id"]); ?>);">审核</a>&nbsp;|&nbsp<?php endif; ?><a href="javascript:void(0);" onclick="select_win(<?php echo ($vo["id"]); ?>);">查看</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></div><div class="btn"><input type="submit" class="button" name="dosubmit" value="删除" onclick="myform.action = '<?php echo U('deleteall');?>';return confirm_deleteall(this.form)"><!-- input type="button" class="button" name="dosubmit" value="删除" onclick="myform.action='<?php echo U('deleteall');?>';return confirm_deleteall() / --></div></form><script language="javascript">    function select_win(id){
    ...
    function fa_award(id){
        art.dialog.open('<?php echo U('award');?>?id='+id,{
		title:'点评审核',
		width:500,
		height:200,
		lock:'true'
