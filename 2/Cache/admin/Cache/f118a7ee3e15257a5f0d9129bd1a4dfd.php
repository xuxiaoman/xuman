<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN"
    ...
    </script></head><body width="100%"><div id="result" class="result none"></div><div class="mainbox"><div id="nav" class="mainnav_title"><ul><a class="on" href="<?php echo U('show_list');?>">地区库</a></ul></div><form name="myform" id="myform" action="" method="post"><div class="table-list"><table cellspacing="0" cellpadding="0" width="100%"><thead><tr><th><a href="javascript:void(0);" onclick="sel_all();">全选</a></th><th style="text-align: left;">名称</th><!--                <?php if($pid == 1): ?><th>国旗</th><?php endif; ?>--><th>英文名称</th><th>排序</th><th>操作</th></tr></thead><tbody><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="ulbc" id="tr_<?php echo ($vo["id"]); ?>"><td id="td_sel" align="center"><input type="checkbox" class="sel" id="sel_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["id"]); ?>" name="items_<?php echo ($vo["id"]); ?>"></td><td style="text-align: left;"><a href="<?php echo U('show_list');?>?pid=<?php echo ($vo["id"]); ?>"><?php echo ($vo["names"]); ?></a></td><!--                    <?php if($pid == 1): ?><td align="center"><img width="50px" height="40px" src="__ROOT__/web/File/flag/<?php echo ($vo["id"]); ?>.gif" /></td><?php endif; ?>--><td align="center"><?php echo ($vo["names_en"]); ?></td><td align="center"><?php echo ($vo["sort"]); ?></td><td align="center"><a href="<?php echo U('show_list');?>?pid=<?php echo ($vo["id"]); ?>"><?php if($vo["levels"] == 1): ?>国家&nbsp;|&nbsp;<?php endif; if($vo["levels"] == 2): ?>省市&nbsp;|&nbsp;<?php endif; if($vo["levels"] == 3): ?>城市&nbsp;|&nbsp;<?php endif; if($vo["levels"] == 4): ?>县/景区&nbsp;|&nbsp;<?php endif; ?></a><a href="javascript:;" onclick="art.dialog.open('<?php echo U('edit');?>?id=<?php echo ($vo["id"]); ?>',{title:'地区编辑',width:550,height:400,window:'top',lock:'true'});">编辑</a>&nbsp;
                <?php if($vo["levels"] > 4): ?>|&nbsp;
                  <a href="javascript:;" onclick="art.dialog.confirm('确定删除吗？',
                  function()
                  {
                     document.location.href='<?php echo U('del');?>?id=<?php echo ($vo["id"]); ?>';
                  })">删除</a></td><?php endif; ?></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></div><div class="btn"><!--  <input type="button" class="button" name="dosubmit" value="删除" onclick="">--><input type="submit" class="button" name="dosubmit" value="删除" onclick="myform.action='<?php echo U('deleteall');?>';"></div></form><div class="page"><?php echo ($page); ?></div><script type="text/javascript">function sel_all(){
