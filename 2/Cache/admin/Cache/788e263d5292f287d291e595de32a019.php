<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN"
    ...
    </script></head><body width="100%"><div id="result" class="result none"></div><div class="mainbox"><div id="nav" class="mainnav_title"><ul><a class="on" href="<?php echo U('area_lists');?>">广告区域管理</a> |
        <a href="<?php echo U('area_add');?>">添加</a></ul></div><form name="myform" id="myform" action="" method="post"><div class="table-list"><table cellspacing="0" cellpadding="0" width="100%"><thead><tr><th><a href="javascript:void(0);" onclick="sel_all();">全选</a></th><th>区域名</th><th>英文标识</th><th>排序</th><th>状态</th><th>操作</th></tr></thead><tbody><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td><input type="checkbox" class="sel" id="sel_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["id"]); ?>" name="items_<?php echo ($vo["id"]); ?>"></td><td><?php echo ($vo["names"]); ?></td><td><?php echo ($vo["names_en"]); ?></td><td><?php echo ($vo["sort"]); ?></td><td><?php echo ($vo["status"]); ?></td><td><a href="<?php echo U('area_edit',array('id'=>$vo['id']));?>">编辑</a>&nbsp;<a href="<?php echo U('area_del',array('id'=>$vo['id']));?>" onclick="return atr_confirm(this.href,'确认删除吗')">删除</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></div><div class="btn"><input type="button" class="button" name="dosubmit" value="删除" onclick="myform.action='<?php echo U('deleteall');?>';return confirm_deleteall()"><input type="button" class="button" name="dosubmit" value="排序" onclick="myform.action='<?php echo U('sort_list');?>';$('#myform').submit();"><?php echo ($page); ?></div></form><script type="text/javascript">
function sel_all(){
