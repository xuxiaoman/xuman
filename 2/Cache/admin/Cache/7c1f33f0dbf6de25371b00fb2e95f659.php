<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN"
    ...
    </script></head><body width="100%"><div id="result" class="result none"></div><div class="mainbox"><div id="nav" class="mainnav_title"><ul><a class="on" href="<?php echo U('show_list');?>">连锁酒店</a> |
        <a href="javascript:;" onclick="art.dialog.open('<?php echo U('add');?>',{title:'添加连锁酒店',width:440,height:300,window:'top',lock:'true'});">添加连锁酒店</a></ul></div><form action="" method="get"><table class="search_table" width="100%"><tr><td class="search">                标题: <input id="title" type="text" class="input-text" name="title" value="<?php echo ($title); ?>">                &nbsp;
                <input type="submit" name="submit" value="查询" class="button"></td></tr></table></form><form name="myform" id="myform" action="" method="post"><div class="table-list"><table cellspacing="0" cellpadding="0" width="100%"><thead><tr><th><a href="javascript:void(0);" onclick="sel_all();">全选</a></th><th style="text-align: left;">名称</th><th>图片</th><th>排序</th><th>操作</th></tr></thead><tbody><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="ulbc" id="tr_<?php echo ($vo["id"]); ?>"><td id="td_sel" align="center"><input type="checkbox" class="sel" id="sel_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["id"]); ?>" name="items_<?php echo ($vo["id"]); ?>"></td><td style="text-align: left;"><input type="text" name="names_<?php echo ($vo["id"]); ?>" id="names_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["names"]); ?>"></td><td align="left" style="width:100px;height:80px"><?php if(($vo["logopath"]) != ""): ?><img src="__ROOT__<?php echo ($vo["logopath"]); ?>" style="width:100px;height:80px"/><?php endif; ?></td><td align="center"><input type="text" name="sort[<?php echo ($vo["id"]); ?>]" id="sort_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["sort"]); ?>"></td><td align="center"><a href="javascript:void(0);" onclick="ajax_save('<?php echo ($vo["id"]); ?>')">保存</a>&nbsp;<a href="javascript:void(0);" onclick="art.dialog.open('<?php echo U('update');?>/id/<?php echo ($vo["id"]); ?>',{title:'编辑连锁酒店',width:440,height:300,window:'top',lock:'true'});">编辑</a>&nbsp;<a href="#" onclick="javascript:if(confirm( '确定要删除该条记录吗？ '))ajax_del(<?php echo ($vo["id"]); ?>);">删除</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></div><div class="btn"><input type="submit" class="button" name="dosubmit" value="删除" onclick="myform.action='<?php echo U('deleteall');?>';"><!-- input type="button" class="button" name="dosubmit" value="删除" onclick="myform.action='<?php echo U('deleteall');?>';return confirm_deleteall() / --><input type="button" class="button" name="dosubmit" value="排序" onclick="myform.action='<?php echo U('sort_list');?>/p/<?php echo ($p); ?>';$('#myform').submit();"></div></form><script language="javascript">    function ajax_save(id){
