...
<script type="text/javascript" src="../Public/js/userhotel.js"></script>
<div >
  <div class="col-19 omega page_content one">
  <div style="margin-top:12px;"></div>
    <li>
      <a href="<?php echo U('hotel_coll');?>/status/1" class="col-3 title <?php if(($status) == "1"): ?>on<?php endif; ?> col-full">已收藏的酒店</a>
      <a href="<?php echo U('hotel_coll');?>/status/0" class="col-3 title <?php if(($status) == "0"): ?>on<?php endif; ?> add">收藏过的酒店</a>
    </li>
    <div class="col-18 alpha body">
      <table width="100%" class="table_list" id="collect-cmd" url="<?php echo U('hotel/ajax_collect');?>">
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr hotel="<?php echo ($vo["hotel_id"]); ?>">
            <td>
              <?php if(($vo["status"]) == "0"): echo ($vo["names"]); ?>
              <?php else: ?>
                <a title='查看<?php echo ($vo["names"]); ?>' href="<?php echo U('hotel/hotel_detailed');?>?hotel=<?php echo ($vo["hotel_id"]); ?>" target="_blank" class="collect_link"><?php echo ($vo["names"]); ?></a><?php endif; ?>
            </td>
            <td style="text-align:center"><?php if(($vo["status"]) == "1"): echo ($vo["create_time"]); else: ?>-<?php endif; ?>
            </td>
            <td>
              <?php if(($vo["status"]) == "1"): ?><a title='点击取消酒店收藏' class='_collect' hotel="<?php echo ($vo["hotel_id"]); ?>" value='0'><span class='collect_on'></span></a>
              <?php else: ?>
                <a title='点击恢复酒店收藏' class='_collect' hotel="<?php echo ($vo["hotel_id"]); ?>" value='1'><span class='collect_off'></span></a><?php endif; ?>
            </td>
