    ...
    $.fn.extend({
        linePlan: function (options) {
            ...
            $(thisObj).bind("change", function () {
                var day = $(this).val();
                var dayreg = /^[1-9]\d*$/;
                var oldday = $("#" + opt.dayId + " option").length;
                if (dayreg.test(day)) {
                    ...
                    if (oldday > day) {
                        ...
                        if (!ismin) {
                            ...
                            for (var di = d; di <= oldday; di++) {
                                ...
                                if (countday)$("#" + opt.conId + di).remove();
                            }
                        }
                    }
                    $("#" + opt.dayId).empty();
            ...
            opt.get_day = function (day) {
                ...
                html = html.replace(/ActivityTitle_\d*/g, "ActivityTitle_" + day);
                html = html.replace(/ActivityTitle_\d*/g, "ActivityTitle_" + day);
                html = html.replace(/TravelText_\d*/g, "TravelText_" + day);
                html = html.replace(/container_\d*/g, "container_" + day);
                html = html.replace(/activity_title_\d*/g, "activity_title_" + day);
                html = html.replace(/activity_text_\d*/g, "activity_text_" + day);
            ...
            opt.get_con = function (con_obj, n) {
                ...
                if (li) {
                    ...
                    var table_selected = $("#" + "container_" + day[1] + "_" + n);
                    return {
                        lists: lists,
                        li: li,
                        tables: tables,
                    ...
                    var create_li = $("<li class='yes'>第<b>" + n + "</b>段</li>");
                    con_obj.append(create_li);
                    var consrc = opt.srcObj.find(".lineRowText table:first").clone();
                    var html = consrc.html();
                    html = html.replace(/activity_title_\d*_\d*/g, "activity_title_" + day[1] + "_" + n);
                    html = html.replace(/activity_text_\d*_\d*/g, "activity_text_" + day[1] + "_" + n);
                    html = html.replace(/第\d*天第\d*段/g, "第" + day[1] + "天第" + n + "段");
            ...
            //删除段数
            $(".del-con").live("click", function (event) {
                var pObj = $(this).parents("ul");
                var pid = pObj.attr("id");
                var day = pid.split("_");
                var n = $(this).parents("ul").find("li:last b").text();
                ...
                var isdel = confirm("您将删除第" + n + "段，一旦删除，会使该段原先编辑的数据也跟着删除，确定删除吗");
                if (!isdel) {
                    return false;
                }
                $(this).parents("ul").find("li:last").remove();
                $("#container_" + day[1] + "_" + n).remove();
                delete opt.kind["kind_activity_text_" + day[1] + "_" + n];
                var cur = $(this).parents("ul").find("li.yes").length;
                if (cur == 0) {
                    $(this).parents("ul").find("li:last").addClass("yes");
                    $("#" + "container_" + day[1] + "_" + (n - 1)).show();
                }
                event.stopPropagation();
            });
            $("." + opt.lineRowSub + " li:gt(1)").live("click", function () {
                var n = $(this).find("b").text();
                $(this).parent().find("li.yes").removeClass("yes");
                $(this).addClass("yes");
                var pObj = $(this).parents("ul");
                var pid = pObj.attr("id");
                var day = pid.split("_");
                $("#TravelText_" + day[1] + " table:visible").hide();
                $("#container_" + day[1] + "_" + n).show();
            });

            opt.showkind = function () {
                $(".linePlan-text").each(function () {
                    var id = "kind_" + $(this).attr("id");
                    if (!(id in opt.kind)) {
                        opt.kind[id] = KindEditor.create(this, $.extend(KindEditor.options, {editorid: id, width: 700, height: 350}));
                    }
                });
            }
            ...
            var init = function () {
                ...
                for (var day in data) {
                    ...
                    for (var d in cons["dining"]) {
                        $("#dining_" + day + "_" + cons["dining"][d]).attr("checked", "checked");

                    }
                    for (var n in cons["con"]) {
                        opt.get_con($("#ActivityTitle_" + day), n);
                        $("#activity_title_" + day + "_" + n).val(cons["con"][n]["title"]);
                        $("#activity_text_" + day + "_" + n).val(cons["con"][n]["content"]);
                    }
                }
                $("." + opt.conClass).hide();
