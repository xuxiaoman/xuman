...
class lineAction extends CommonAction {
    ...
    public function show_list() {
        ...
        if ($_GET && !$_GET['p']) {
            ...
            if ($_GET['target_type'] != '') {
                $where['target_type'] = array("eq", $_GET['target_type']);
            }
            if (!empty($_GET['sort'])) {
    ...
    public function edit() {
        ...
        if (!$_POST) {
            ...
            $start_city = M()->table($table_city)
                    ->join($table_area . " on city.cid=area.id")
                    ->where("types='LINE'")->field("city.*,area.names")
                    ->order("city.sort")
                    ->select();
    ...
    public function getdata() {
        $data = array(
            "20130924" => array("2000", "1800", "1500"),
            "20131001" => array("2000", "1800", "1500"),
        );
        $this->ajaxReturn($data);
    }
    ...
    public function pic_list() {
        ...
        $this->assign("add_url", U('line/pic_add', array('line_id' => $line_id)));
        $this->assign('list', $list);
    ...
    public function pic_add() {
        if (!isset($_POST['save'])) {
            ...
            $LinePic->add();
            $this->success("保存成功", U('pic_list', array('line_id' => $_GET["line_id"])));
        }
    }
    ...
    public function ajax_target() {
        ...
        $map['target.start_id'] = array('eq', $start);
        $map['target.type_id'] = array('eq', $type);
    ...
    private function json_price_list($arr) {
        $json_arr = array();
        foreach ($arr as $k => $v) {
            $json_arr[date("Ymd", $k)] = array($v["RACKRATE"], $v["price_adult"], $v["price_children"]);
        }
    ...
    public function is_code() {
        $param = $_POST["param"];
        $count = M("line")->where(array("code" => $param))->count();
        if ($count > 0) {
            $this->ajaxReturn(array("info" => "编号已经存在！", "status" => "n"));
        } else {
            $this->ajaxReturn(array("info" => "编号可以使用！", "status" => "y"));
        }
    }
